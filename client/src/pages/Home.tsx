...
import "./PhotoLightboxRefinement.css";
...
const memberPhotos: Record<string, string> = {
  Al: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663649873365/sdTACTKcYWTiDvgx.jpg",
};
type GalleryItem = { code: string; image: string; caption?: string };
...
export default function Home() {
  ...
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") { setSelectedCard(null); setSelectedGallery(null); setMenuOpen(false); }
    };
  ...
  useEffect(() => {
    ...
    const cards = Array.from(document.querySelectorAll<HTMLElement>("#members .member-scroll-reveal"));
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
    ...
    }, { threshold: 0.34, rootMargin: "0px 0px -7% 0px" });
  ...
  return (
    <div className="class-site" style={{ "--hero-image": `url(${HERO_IMAGE_URL})` } as CSSProperties}>
      ...
      <main>
        ...
        <section className="members-section content-scene" id="members" aria-labelledby="members-title"><video ref={membersVideoRef} className="members-video" muted loop playsInline preload="none" aria-hidden="true"><source src={GALLERY_VIDEO_URL} type="video/mp4" /></video>{!isMobile && <SpaceBackground mode="gallery" />}<div className="geometry-layer member-geometry scroll-parallax" data-parallax="0.62" aria-hidden="true"><i /><i /><i /></div><div className="scene-title reveal-item"><OrbitMark /><h2 id="members-title">XII-H</h2></div><div className="teacher-panel reveal-item"><div className="teacher-photo-frame" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M4 5.5h5l1.3-1.7h3.4L15 5.5h5v13H4v-13Z" stroke="currentColor" strokeWidth="1.2"/><circle cx="12" cy="12" r="3.4" stroke="currentColor" strokeWidth="1.2"/></svg></div><span className="teacher-panel__label">WALI KELAS</span><button type="button" onClick={() => setSelectedCard("Wali Kelas")} aria-label="Buka kartu wali kelas"><ArrowUpRight /></button></div><div className="member-deck" aria-label="Kartu anggota kelas">{members.map((name, index) => { const photo = memberPhotos[name]; return <button className={`member-card member-scroll-reveal card-${index % 4}${photo ? " has-photo" : ""}`} data-parallax={0.14 + (index % 4) * 0.05} style={{ "--reveal-delay": `${index * 38}ms`, "--member-reveal-delay": `${(index % 6) * 72}ms` } as CSSProperties} key={name} type="button" onClick={() => setSelectedCard(name)} aria-label={`Buka profil ${name}`}><span>{String(index + 1).padStart(2, "0")}</span><i className={photo ? "member-photo" : undefined} style={photo ? { backgroundImage: `url(${photo})` } : undefined} /><b>{name}</b></button>; })}</div></section>

        <section className="gallery-section content-scene" id="gallery" aria-labelledby="gallery-title"><div className="geometry-layer gallery-geometry scroll-parallax" data-parallax="0.48" aria-hidden="true"><i /><i /><i /></div><div className="scene-title reveal-item"><OrbitMark /><h2 id="gallery-title">Galeri</h2></div><div className="gallery-marquee" aria-label="Arus foto Galeri XII-H"><div className="gallery-marquee__row gallery-marquee__row--top"><div className="gallery-marquee__track gallery-marquee__track--right">{galleryForward.map((item, index) => <button className="gallery-strip-card" style={{ "--reveal-delay": `${Math.min(index, 5) * 65}ms` } as CSSProperties} key={`forward-${index}`} type="button" onClick={() => setSelectedGallery(item)} aria-label={`Buka foto ${item.code}`}><img src={item.image} alt="" loading="lazy" /><span>{item.code}</span></button>)}</div></div><div className="gallery-marquee__row gallery-marquee__row--middle"><div className="gallery-marquee__track gallery-marquee__track--left">{galleryBackward.map((item, index) => <button className="gallery-strip-card" style={{ "--reveal-delay": `${Math.min(index, 5) * 65 + 130}ms` } as CSSProperties} key={`middle-${index}`} type="button" onClick={() => setSelectedGallery(item)} aria-label={`Buka foto ${item.code}`}><img src={item.image} alt="" loading="lazy" /><span>{item.code}</span></button>)}</div></div><div className="gallery-marquee__row gallery-marquee__row--bottom"><div className="gallery-marquee__track gallery-marquee__track--right gallery-marquee__track--slow">{galleryForward.map((item, index) => <button className="gallery-strip-card" style={{ "--reveal-delay": `${Math.min(index, 5) * 65 + 240}ms` } as CSSProperties} key={`bottom-${index}`} type="button" onClick={() => setSelectedGallery(item)} aria-label={`Buka foto ${item.code}`}><img src={item.image} alt="" loading="lazy" /><span>{item.code}</span></button>)}</div></div></div></section>
      ...
      {selectedCard && <div className="profile-layer" role="dialog" aria-modal="true" aria-labelledby="profile-layer-title" onClick={() => setSelectedCard(null)}><div className="profile-panel" onClick={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="Tutup" onClick={() => setSelectedCard(null)}>×</button><p className="eyebrow"><OrbitMark /> XII-H</p><h2 id="profile-layer-title">{selectedCard}</h2>{memberPhotos[selectedCard] ? <img className="profile-member-photo" src={memberPhotos[selectedCard]} alt={`Foto profil ${selectedCard}`} /> : <span className="profile-empty">Data menyusul</span>}</div></div>}
      {selectedGallery && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-labelledby="gallery-lightbox-title" onClick={() => setSelectedGallery(null)}><figure className="gallery-lightbox__panel" onClick={(event) => event.stopPropagation()}><button className="gallery-lightbox__close" type="button" aria-label="Tutup foto" onClick={() => setSelectedGallery(null)}>×</button><img src={selectedGallery.image} alt={`Foto Galeri XII-H nomor ${selectedGallery.code}`} /><figcaption><strong id="gallery-lightbox-title">Foto {selectedGallery.code}</strong>{selectedGallery.caption && <span>{selectedGallery.caption}</span>}</figcaption></figure></div>}
    </div>
  );
}
